/**
 * MCP Configuration - Extends existing config system
 * Integrates with the main config/env.js for consistency
 */
...
// Load the main config synchronously
let mainConfig;
try {
  // Use dynamic import in a synchronous way
...
const mcpConfig = {
  ...
  auth: {
    ...
    internal: {
      ...
      allowedEmails: (process.env.MCP_ALLOWED_EMAILS || '').split(',').filter(Boolean),
