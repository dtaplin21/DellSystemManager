...
-- Create enum for the six domains (idempotent)
DO $$ BEGIN
...
-- Create the main asbuilt records table (idempotent)
...
CREATE INDEX IF NOT EXISTS idx_asbuilt_confidence ON asbuilt_records(ai_confidence);
CREATE INDEX IF NOT EXISTS idx_asbuilt_requires_review ON asbuilt_records(requires_review);
CREATE INDEX IF NOT EXISTS idx_asbuilt_created_at ON asbuilt_records(created_at);

-- Add comments for documentation (idempotent - comments can be updated)
...
-- Create function to update updated_at timestamp (idempotent - CREATE OR REPLACE)
...
DROP TRIGGER IF EXISTS update_asbuilt_records_updated_at ON asbuilt_records;
