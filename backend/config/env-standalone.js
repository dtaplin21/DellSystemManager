...
// Self-contained environment configuration for backend
// This avoids path resolution issues in deployment environments
...
const EnvSchema = z
  ...
  .passthrough()
  .superRefine((env, ctx) => {
    if (!env.SUPABASE_SERVICE_ROLE_KEY && !env.SUPABASE_KEY && !env.SUPABASE_ANON_KEY) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['SUPABASE_SERVICE_ROLE_KEY'],
        message: 'Either SUPABASE_SERVICE_ROLE_KEY, SUPABASE_KEY, or SUPABASE_ANON_KEY must be provided'
      });
    }
  });
...
const configuration = {
  ...
  supabase: {
    ...
    anonKey: env.SUPABASE_KEY || env.SUPABASE_ANON_KEY || null,
